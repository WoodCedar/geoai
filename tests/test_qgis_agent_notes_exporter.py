...
def test_write_agent_notes_creates_project_index_layer_cards_and_agent_context(tmp_path):
    ...
    assert (output_dir / "project.md").exists()
    ...
    assert "[工程说明](project.md)" in index_text
    ...
    assert "这是 QGIS 工程的 Markdown 索引" in agent_text
    assert "湖泊 边界 -> layers/湖泊_边界.md" in agent_text
    assert "原始地图数据仍以 QGIS 工程和图层源文件为准" in agent_text
